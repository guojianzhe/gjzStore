						...
						<tbody>
							...
							<c:forEach items="${cart.map }" var="item" >
								...
								<tr class="active">
									...
									<td width="10%">
										<input type="text" name="count" value="${item.value.count}" maxlength="4" size="10">
									</td>
									<td width="15%">
										<span class="subtotal">￥${item.value.subtotal}</span>
									</td>
									<td>
										<a href="${pageContext.request.contextPath}/cart?method=remove&pid=${item.key}" class="delete">删除</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
